...
The first line of the generated file records its source, with local paths
relative to the output file's directory, e.g. `// Generated by
@fbritoferreira/strapi generate from dir ../../my-strapi. Do not edit.` There
is no timestamp, so regenerating an unchanged schema leaves the file as it was
and the same checkout produces the same file on every machine. `--check`
ignores that line.
