...
describe("emit", () => {
	it("matches the committed snapshot for the fixture project", async () => {
		const output = emit(normalize(await loadFromDir(project), { includePlugins: false }), { source: "fixture project" });
	...
	it("starts with the header and a minimal import", () => {
		...
		const out = emit(model, { source: "x" });
		expect(out.split("\n")[0]).toBe("// Generated by @fbritoferreira/strapi generate from x. Do not edit.");
	...
	it("lists populatable fields on a marker property", () => {
		...
		expect(emit(model, { source: "x" })).toContain('\treadonly __populatable?: "author" | "tags";');
	});
	...
	it("omits the relations marker when a type has only components", () => {
		...
		expect(emit(model, { source: "x" })).not.toContain("__relations");
	});
	...
	it("omits the marker when nothing is populatable", () => {
		...
		expect(emit(model, { source: "x" })).not.toContain("__populatable");
	});
	...
	it("quotes field names that are not identifiers", () => {
		...
		expect(emit(model, { source: "x" })).toContain('\t"kebab-case"?: string;');
	});
});
